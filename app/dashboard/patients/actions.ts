...
// fields so later template edits never change an already-issued
// certificate), then render and store the PDF right away — best-effort,
// since a PDF failure shouldn't undo an otherwise-valid, numbered
// certificate.
export async function issueMedicalCertificateAction(patientId: string, templateId: string, values: Record<string, string>): Promise<string> {
  const { profile } = await requireClinicMember();
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("issue_medical_certificate", {
    p_patient_id: patientId,
    p_template_id: templateId,
    p_values: values,
  });
  if (error) throw new Error(error.message);
  const certificateId = data as string;

  try {
    const pdfData = await gatherMedicalCertificatePdfData(supabase, profile.tenant_id, certificateId);
    const pdfBuffer = await renderToBuffer(MedicalCertificateDocument({ data: pdfData }));
    await supabase.storage.from("medical-certificates").upload(`${certificateId}/certificate.pdf`, pdfBuffer as any, { contentType: "application/pdf" });
  } catch {
    // Certificate record + number stand either way — the PDF can be
    // regenerated by re-fetching this same signed URL once storage/PDF
    // rendering succeeds on a retry.
  }
...
export async function voidMedicalCertificateAction(certificateId: string, patientId: string, reason: string) {
  const { profile } = await requireClinicMember();
  const supabase = await createClient();
  const { error } = await supabase.rpc("void_medical_certificate", { p_certificate_id: certificateId, p_reason: reason || null });
  if (error) throw new Error(error.message);

  // Re-render so the stored PDF also shows the VOID watermark — same
  // best-effort handling as issuance.
  try {
    const pdfData = await gatherMedicalCertificatePdfData(supabase, profile.tenant_id, certificateId);
    const pdfBuffer = await renderToBuffer(MedicalCertificateDocument({ data: pdfData }));
    await supabase.storage.from("medical-certificates").upload(`${certificateId}/certificate.pdf`, pdfBuffer as any, { contentType: "application/pdf", upsert: true });
  } catch {
    // Non-fatal — the certificate's status is void either way.
  }

  revalidatePath(`/dashboard/patients/${patientId}`);
}

export async function getMedicalCertificatePdfUrlAction(certificateId: string): Promise<string> {
  await requireClinicMember();
  const supabase = await createClient();
  const { data, error } = await supabase.storage.from("medical-certificates").createSignedUrl(`${certificateId}/certificate.pdf`, 300);
  if (error || !data?.signedUrl) throw new Error("No PDF is available for this certificate yet — try again in a moment.");
  return data.signedUrl;
}
